...
fn main() {
    ...
    if args.len() < 2 {
        println!("Usage:
    - get-palette [image_path] [palette_size(opt, 2 as min)]: extract palette from the image
    ...
    if args[1] == "get-palette" {
        ...
        process_frames_mod(
            &DitheringOrdered {},
            &ProcessingArgs::DitheringOrdered(args[2].parse::<f32>().unwrap()),
        )
        .unwrap();
    } else if args[1] == "quadrate" {
        process_frames_new(
            &Quadrate {},
            &ProcessingArgs::Quadrate(args[2].parse::<f32>().unwrap()),
        )
        .unwrap();
    } else if args[1] == "downscale" {
        process_frames_new(
            &Downscale {},
            &ProcessingArgs::Downscale(args[2].parse::<u32>().unwrap()),
        )
        .unwrap();
    } else if args[1] == "upscale" {
        process_frames_new(
            &Upscale {},
            &ProcessingArgs::Upscale(args[2].parse::<u32>().unwrap()),
        )
        .unwrap();
    }
}
